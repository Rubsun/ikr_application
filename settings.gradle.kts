pluginManagement {
    repositories {
        google {
            content {
                includeGroupByRegex("com\\.android.*")
                includeGroupByRegex("com\\.google.*")
                includeGroupByRegex("androidx.*")
            }
        }
        mavenCentral()
        gradlePluginPortal()
    }
    plugins {
        id("org.jetbrains.kotlin.plugin.compose") version "2.0.21"
    }
}
dependencyResolutionManagement {
    repositoriesMode.set(RepositoriesMode.FAIL_ON_PROJECT_REPOS)
    repositories {
        google()
        mavenCentral()
        maven { url = uri("https://jitpack.io") }
    }
}

rootProject.name = "ikr_application"
include(":app")
include(":features:dyatlova:api")
include(":features:dyatlova:impl")
include(":features:nastyazz:api")
include(":features:nastyazz:impel")
include(":libs:injector")
include(":features:grigoran:api")
include(":features:grigoran:impl")
include(":features:rin2396:api")
include(":features:rin2396:impl")
include(":features:stupishin:api")
include(":features:stupishin:impl")
include(":features:n0tsszzz:api")
include(":features:n0tsszzz:impl")
include(":features:artemkaa:api")
include(":features:artemkaa:impl")
include(":features:antohaot:api")
include(":features:antohaot:impl")
include(":features:drain678:api")
include(":features:drain678:impl")
include(":libs:primitivestorage:api")
include(":libs:primitivestorage:data")
include(":libs:egorik4-network:api")
include(":libs:egorik4-network:data")
include(":libs:artemkaa-network:api")
include(":libs:artemkaa-network:data")
include(":libs:antohaot-network:api")
include(":libs:antohaot-network:data")
include(":libs:drain678-network:api")
include(":libs:drain678-network:data")
include(":features:denisova:api")
include(":features:denisova:impl")
include(":libs:dyatlova-network")
include(":features:rin2396:api")
include(":features:rin2396:impl")
include(":features:antohaot:api")
include(":features:antohaot:impl")
include(":features:drain678:api")
include(":features:drain678:impl")
include(":features:dimmension:api")
include(":features:dimmension:impl")
include(":features:zagora:api")
include(":features:zagora:impl")
include(":features:MomusWinner:api")
include(":features:MomusWinner:impl")
include(":features:michaelnoskov:api")
include(":features:michaelnoskov:impl")
include(":features:egorik4:api")
include(":features:egorik4:impl")
include(":features:alexcode69:api")
include(":features:alexcode69:impl")
include(":features:demyanenko:impl")
include(":features:demyanenko:api")
include(":features:rubsun:api")
include(":features:rubsun:impl")
include(":features:tire:api")
include(":features:tire:impl")
include(":features:eremin:api")
include(":features:eremin:impl")
include(":features:akiko23:api")
include(":features:akiko23:impl")
include(":features:quovadis:api")
include(":features:quovadis:impl")
include(":features:spl3g:api")
include(":features:spl3g:impl")
include(":features:kristevt:impl")
include(":features:kristevt:api")
include(":libs:roomstorage:api")
include(":libs:roomstorage:data")
include(":libs:catLover:data")
include(":libs:catLover:api")
include(":libs:demyanenkoCoil:impl")
include(":libs:demyanenkoCoil:api")
include(":libs:demyanenkoOpenF1:impl")
include(":libs:logger:api")
include(":libs:logger:timber")
include(":libs:logger:stub")
include(":libs:imageloader:api")
include(":libs:imageloader:data")
include(":libs:network:api")
include(":libs:network:data")
include(":libs:alexcode69-network:api")
include(":libs:alexcode69-network:data")
include(":libs:capybara-network:api")
include(":libs:capybara-network:data")
include(":libs:nastyazz:api")
include(":libs:nastyazz:data")
include(":libs:jikan:api")
include(":libs:jikan:data")
include(":libs:chart:api")
include(":libs:chart:data")
include(":libs:n0tsszzz-network:api")
include(":libs:n0tsszzz-network:data")
include(":libs:lyrics:api")
include(":libs:lyrics:impl")
include(":libs:dimmension-imageloader:api")
include(":libs:dimmension-imageloader:data")
include(":libs:dimmension-network:api")
include(":libs:dimmension-network:data")
include(":libs:spl3g-network:api")
include(":libs:spl3g-network:data")
include(":libs:michaelnoskov-network:api")
include(":libs:michaelnoskov-network:data")
include(":libs:michaelnoskov-chart:api")
include(":libs:michaelnoskov-chart:data")
include(":libs:rubsun-chart:api")
include(":libs:rubsun-chart:data")
include(":libs:rubsun-network:api")
include(":libs:rubsun-network:data")
include(":libs:rubsun-storage:api")
include(":libs:rubsun-storage:data")
include(":libs:akiko23-network:api")
include(":libs:akiko23-network:data")
include(":libs:coil:api")
include(":libs:coil:data")
include(":libs:tire-network:api")
include(":libs:tire-network:data")

include(":libs:momuswinner-network:api")
include(":libs:momuswinner-network:data")
include(":libs:momuswinner-chart:api")
include(":libs:momuswinner-chart:data")
include(":libs:grigoran-network:api")
include(":libs:grigoran-network:data")
include(":libs:tire-storage:api")
include(":libs:tire-storage:data")
include(":features:vtyapkova:api")
include(":features:vtyapkova:impl")
include(":libs:vtyapkova-network:api")
include(":libs:vtyapkova-network:data")
include(":features:argun:api")
include(":features:argun:impl")
include(":libs:argun-network:api")
include(":libs:argun-network:data")
include(":libs:fomin-network:api")
include(":libs:fomin-network:data")
include(":features:fomin:api")
include(":features:fomin:impl")
include(":libs:arch")
include(":features:screens:api")
include(":features:screens:impl")
include(":features:telegin:api")
include(":features:telegin:impl")
include(":libs:telegin-network:api")
include(":libs:telegin-network:data")
include(":libs:telegin-storage:api")
include(":libs:telegin-storage:data")
